...
# Central place where I (try to) place all completions Zsh.
fpath=("{{ zsh_completions }}" $fpath)

# Central place where I (try to) place all functions for Zsh.
...
for plugin ($plugins); do
    if is_plugin $_omz $plugin; then
        fpath=($_omz/plugins/$plugin $fpath)
    else
        echo "OMZ plugin '$plugin' not found."
    fi
done

for plugin ($plugins); do
    if [ -f $_omz/plugins/$plugin/$plugin.plugin.zsh ]; then
